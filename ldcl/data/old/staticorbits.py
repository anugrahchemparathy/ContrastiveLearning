...
def orbits_train_gen(batch_size, traj_samples=100, noise=0., shuffle=True, check=False, H=None, L=None, phi0=None):
    ...
    while E is None:
        ...
        H = -mu / 2 * (0.5 + 0.5 * rng.uniform(size=(batch_size, 1))) if H is None else H * np.ones((batch_size, 1)) #energy
        L = 0.5 * np.ones((batch_size, 1))
        #rng.uniform(size=(batch_size, 1)) if L is None else L * np.ones((batch_size, 1)) #angular momentum
        ...
        phi0 = np.pi * np.ones((batch_size, 1))
        #2 * np.pi * rng.uniform(size=(batch_size, 1)) if phi0 is None else phi0 * np.ones((batch_size, 1))
