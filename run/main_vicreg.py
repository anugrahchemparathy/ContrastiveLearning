...
from ldcl.losses.vicreg import vicreg_loss
...
def training_loop(args):
    ...
    if is_natural:
        ...
        encoder = branch.branchEncoder(encoder_out=3)
        projector = branch.projectionHead(head_in = 3, head_out = 16)

    model = branch.sslModel(encoder=encoder, projector = projector)
    model.to(device)
    ...
    def apply_loss(z1, z2, loss_func = vicreg_loss):
        loss = vicreg_loss(z1,z2, l = 1, m = 1)
        return loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fname', default='vicreg_test' , type = str)
